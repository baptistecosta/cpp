...
{
	const float c_x = cos(radians(x)), c_y = cos(radians(y)), c_z = cos(radians(z)),
				s_x = sin(radians(x)), s_y = sin(radians(y)), s_z = sin(radians(z));

	switch (rot_order)
	{
		case RotOrder_XZY:
			return Matrix3	(	c_y * c_z,	s_x * s_y + c_x * c_y * s_z,	-c_x * s_y + c_y * s_x * s_z,
								-s_z,		c_x * c_z,						c_z * s_x,
								c_z * s_y,	-c_y * s_x + c_x * s_y * s_z,	c_x * c_y + s_x * s_y * s_z	);
		case RotOrder_ZYX:
			return Matrix3	(	c_y * c_z,						c_y * s_z,						-s_y,
								c_z * s_x * s_y - c_x * s_z,	c_x * c_z + s_x * s_y * s_z,	c_y * s_x,
								c_x  *c_z * s_y + s_x * s_z,	-c_z * s_x + c_x * s_y * s_z,	c_x * c_y	);

		case RotOrder_XYZ:
			return Matrix3	(	c_y * c_z,		c_z * s_x * s_y + c_x * s_z,	-c_x * c_z * s_y + s_x * s_z,
								-c_y * s_z,		c_x * c_z - s_x * s_y * s_z,	c_z * s_x + c_x * s_y * s_z,
								s_y,			-c_y * s_x,						c_x * c_y	);

		case RotOrder_ZXY:
			return Matrix3	(	c_y * c_z - s_x * s_y * s_z,	c_z * s_x * s_y + c_y * s_z,	-c_x * s_y,
								-c_x * s_z,						c_x * c_z,						s_x,
								c_z * s_y + c_y * s_x * s_z,	-c_y * c_z * s_x + s_y * s_z,	c_x * c_y	);

		case RotOrder_YZX:
			return Matrix3	(	c_y * c_z,						s_z,		-c_z * s_y,
...
{
	return Matrix3 (	mat.m[0],  mat.m[1],  mat.m[2],
