...
const float BCosta::Core::Math::radians(const float degrees)
{ return degrees / 180.f * pi; }

const float degrees(const float radians)
{ return radians / pi * 180.f; }

void persp(Matrix4 &m, const float fov, const float ratio, const float z_near, const float z_far)
{
...
void ortho(Matrix4 &m, const float l, const float r, const float b, const float t, const float n, const float f)
{
...
void lookAt(Matrix4 &m, Vector3 &pos, const Vector3 &dir, const Vector3 &up)
{
