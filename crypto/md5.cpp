...
//---------------------------------------------------------------------------
void			MD5::Compute(const uchar* data)
{
	uint	a = abcd[0],
			b = abcd[1],
			c = abcd[2],
	...
	if (*((const uchar *)&w))	// dynamic little-endian
#endif
...
//---------------------------------------------------------------------------
void			MD5::Append(const uchar* data, int nbytes)
{
	const uchar* p = data;
	int left = nbytes;
...
//---------------------------------------------------------------------------
void			MD5::Finish(uchar digest[16])
{
	...
	Append(data, 8);
	for (int i = 0; i < 16; ++i)
		digest[i] = (uchar)(abcd[i >> 2] >> ((i & 3) << 3));
}

//---------------------------------------------------------------------------
void			MD5::Process(__i const void* data, __i const int data_len, __o uchar* hash)
{
	MD5 md5;
	md5.Append((const uchar*)data, data_len);
	md5.Finish(hash);
}
